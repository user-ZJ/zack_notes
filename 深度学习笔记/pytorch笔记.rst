...
历史版本安装 ： https://pytorch.org/get-started/previous-versions/#v182-with-lts-support
...
.. code-block:: python 
	...
	x.size()  #获取矩阵的shape
	torch.randint(0, 100, (1, 123), dtype=torch.int64)
...
.. code-block:: python 
	...
	class Wav2VecExport(torch.nn.Module):
		def __init__(self, wav2vecmodel):
			super().__init__()
			self.wav2vecmodel = wav2vecmodel
		

		def forward(self, feature):
			outputs = self.wav2vecmodel(feature)
			return outputs.last_hidden_state

	wav2vec_model = Wav2Vec2Model.from_pretrained(model_path)
	wav2vecexport = Wav2VecExport(wav2vec_model.cpu())
	input = torch.randn(1, 10240, requires_grad=True)
	torch.onnx.export(wav2vecexport,         # model being run
				...
				dynamic_axes={'input' : {1 : 'sample_num'}})
