...
除了经典的 GOF 23 种设计模式外，实际项目中还经常遇到一些补充模式，它们不一定属于 GOF 原始分类，但对架构设计和代码复用也很有价值：

- 创建型扩展：空对象模式（Null Object）、对象池模式、依赖注入/控制反转（DI/IoC）、惰性初始化、工厂注册模式。
- 结构型扩展：过滤器模式、职责链模式、服务定位器模式、事件总线/发布-订阅模式、MVC/MVP/MVVM 架构、数据访问对象（DAO）模式、业务代表模式。
- 行为型扩展：状态机模式、解释器模式、责任链模式、策略组合、事件驱动模式、命令队列、观察者事件、访问者模式。
- 并发与架构类补充：生产者-消费者、线程池、工作窃取、发布-订阅、CQRS、领域驱动设计（DDD）、微服务模式等。

这些模式可以视项目需求补充到现有设计模式库中，帮助理解系统的可扩展性、复用性、解耦以及性能优化。
