...
- 禁止拷贝与赋值：除了禁止复制构造函数和复制赋值运算符，还应当禁止移动构造函数和移动赋值运算符，防止通过拷贝或移动创建第二个实例。
- 线程安全与双检锁：经典的双检锁在旧版本 C++ 和 Java 中可能因为内存重排而失效。C++11 起，函数内静态局部变量的初始化具有线程安全性，`std::call_once` 也提供了安全且简洁的单例初始化方式。
- 生命周期与析构顺序：静态单例对象的析构顺序在程序退出时不一定可控，会出现静态终结顺序问题。对于依赖于其他静态对象的单例，应当谨慎设计销毁策略，或者选择“永不过期”的单例。
- 全局状态与测试性：单例本质上是一种全局状态，可能降低模块的可测试性和可维护性。过度使用单例会增加隐式依赖和耦合度，因此在复杂系统中应该优先考虑依赖注入或显式对象管理。
- 多例与变种：如果一个类需要多个受控实例，可以考虑改用多例模式（Multiton）或者工厂模式，而不是直接把所有对象都设计成单例。
